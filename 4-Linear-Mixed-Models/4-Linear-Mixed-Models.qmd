...
knitr::include_graphics("4-Abbildungen/ivf-4-otto.png")
```
\vspace{2mm}
...
\center
\huge
\textcolor{black}{(4) Linear Mixed Models}
\vfill
...
write.csv(D, "./4-Daten/mz-anova.csv", row.names = FALSE)           # Speichern
```
\vspace{1mm}
Beispieldatensatz
```{r}
fname = "./4-Daten/mz-anova.csv"
...
```{r, eval = F}
pdf(
file        = "./4-Abbildungen/ivf-4-mz-anova.pdf",
width       = 7,
height      = 5)
par(
family      = "sans",
...
knitr::include_graphics("4-Abbildungen/ivf-4-mz-anova.pdf")
```
...
\item $X_i \in \mathbb{R}^{n_i \times p}$ eine vorgegebene Matrix ist, die \textit{Fixed-Effects-Designmatrix des $i$ten Clusters} genannt wird,
...
\item $Z_i \in \mathbb{R}^{n_i \times q_i}$ eine vorgegebene Matrix ist, die \textit{Random-Effects-Designmatrix des $i$ten Clusters} genannt wird,
...
\end{equation}
\item $\eps_i$ ein $n_i$-dimensionaler latenter Zufallsvektor ist, der \textit{Zufallsfehler des $i$ten Clusters} genannt wird, mit
\begin{equation}
\eps_i \sim N(0_{n_i}, \Sigma_{\eps_i}) \mbox{ mit }  \Sigma_{\eps_i} \in \mathbb{R}^{n_i \times n_i} \mbox{ p.d.  und unabhängig von } b_j \mbox{ für } j = 1,...,k.
\end{equation}
\end{itemize}
...
und mit
\begin{equation}
\Sigma_b := \mbox{blockdiag}(\Sigma_{b_1},...,\Sigma_{b_k})
\mbox{ und }
\Sigma_{\eps} := \mbox{blockdiag}(\Sigma_{\eps_1},...,\Sigma_{\eps_k}).
\end{equation}
\end{definition}
Bemerkungen
...
\footnotesize
* Grundidee: Modellierung der Daten durch ein Zweistichproben-T-Test-Modell mit zufälligem HSA-Effekt
...
y_{i2j} & = \mu_0 + \alpha_2 + \mu_i + \eps_{i2j}       \mbox{ mit } \eps_{i2j} \sim N(0,\sigma^2_{\eps}) \mbox{ für } j = 1,...,n_{i_2}
\end{split}
\end{align}
...
\begin{equation}\label{eq:lemi-example}
y_i = X_i\beta + Z_ib_i + \eps_i \mbox{ mit } \eps_i \sim N(0_{n_i}, \sigma_\eps^2 I_{n_i})
\end{equation}
mit
\begin{equation}
y_i =
...
\begin{equation}
y = X\beta + Zb + \eps  \mbox{ mit } \eps \sim N(0_n,\sigma_\eps^2 I_n) \mbox{ und } b \sim N(0_4, \sigma^2_bI_4)
\end{equation}
sowie
\tiny
\setstretch{1.0}
...
\eqref{eq:lmm-gen} mit den \textit{Populationsparametern} $\beta \in \mathbb{R}^p$ und $\Sigma_b \in \mathbb{R}^{q \times q}$.
\end{theorem}
Bemerkungen

* In \eqref{eq:lmm-pop} denkt man sich $u$ aus einer Population mit Erwartungswert $A\beta$ realisiert.
* Der Fixed-Effects-Parameter $\beta$ parameterisiert dabei explizit den Erwartungswert dieser Population.
...
\beta := \beta,
\sigma^2 := 1,
V :=  \sigma^2_{b}ZZ^T + \sigma_\eps^2 I_n \mbox{ und somit }
\sigma^2V = V = V_\theta.
\end{equation}
...
& = N\left(0_n, \sigma^2K^{-1}V(K^{-1})^T\right)      \\
& = N\left(0_n, \sigma^2K^{-1}KK^T(K^{-1})^T\right)      \\
& = N\left(0_n, \sigma^2K^{-1}K(K^{-1}K)^T\right)      \\
...
\footnotesize
Leider funktioniert die vorgeschlagene Wahl von $M$ in dieser Form nicht, da $\mbox{rg}(M) = n-p < n$ und $MV_\theta M^T$ damit singulär ist.
...
\begin{equation}
\ln |KV_\theta K^T| = \ln |V_\theta | + \ln |X^TV_\theta^{-1}X| + c_K
\end{equation}
und
\begin{equation}
(Ky)^T(KV_\theta K^T)^{-1}Ky = (y-X\hat{\beta})^T V_\theta^{-1} (y-X\hat{\beta})
\end{equation}
mit einer nicht von $\theta$ abhängigen Konstante $c_K$. Damit ergibt
sich für die Log-Likelihood-Funktion von $z = Ky$ aber, dass
\begin{equation}
\ell(\theta) = -\frac{m}{2}\ln 2\pi - \frac{1}{2}\ln |V_\theta | - \frac{1}{2} \ln |X^TV_\theta^{-1}X| - \frac{1}{2}(y-X\hat{\beta})^T V_\theta^{-1} (y-X\hat{\beta}) + c_K
\end{equation}
...
* Wie verhält sich die Fehlerkontrastmotivation zur Expectation-Maximization Motivation (vgl. @laird1982)?
...
llh_reml = function(theta, y, X, Z, beta_hat){
    ...
    # Zielfunktion für das Random-Effects-Modell der Metaanalyse.
    #
    # Inputs
...
Parameterwerte wiederum durch ihre Schätzer ersetzt werden.
...
estimate = function(lmm){
    ...
    for (k in 2:K){                                                             # Iterationen
        ...
        max_l_reml      = optim(par=theta_hat_k[,k-1], fn=reml, lmm=lmm,        # ReML-Varianzkomponentschätzung
                                method = "L-BFGS-B",
                                lower = rep(.Machine$double.eps, c))
...
D           = read.csv("./4-Daten/mz-anova.csv", head = T)          # Dataframe
...
\begin{equation}
\hat{\beta} \stackrel{a}{\sim} N\left(\beta, \left(X^TV_{\hat{\theta}}^{-1}X \right)^{-1}\right).
\end{equation}
...
evaluate = function(lmm){
    ...
    # Diese Funktion evaluiert ein geschätztes LMM.
    #   Inputs:
    #       .lmm    : R Liste mit Einträgen
    #        $y          : n x 1 Datenvektor
    #        $X          : n x p Fixed-Effects-Designmatrix
